...
⚠️ 학과명은 반드시 아래 목록에서만 선택하세요. 임의로 만들지 마세요:
...
⚠️ IT공과대학은 컴퓨터공학부·기계전자공학부·산업시스템공학부를 통합 모집합니다. "컴퓨터공학부", "전자공학부", "정보통신공학부" 같은 세부 학과명은 존재하지 않으므로 절대 사용하지 마세요. "공대", "IT쪽", "이공계", "컴공" 등은 모두 "IT공과대학"으로 매핑하세요.
...
⚠️ 중요: 한성대학교는 수시·정시 모두 문과/이과 교차지원이 가능합니다.
이과 학생도 문과 계열 학과에 지원할 수 있고, 문과 학생도 이과 계열 학과에 지원할 수 있습니다.
단, 교과우수전형은 반영교과가 계열별로 다르고, 정시는 수능 반영비율이 다르므로 유불리를 고려해야 합니다.

이과(자연계/공학계) 계열에 유리한 학과:
  - IT공과대학(컴퓨터공학부·기계전자공학부·산업시스템공학부 통합모집) - 수능 수학35% / 교과우수 과학 반영
  - AI응용학과 - 수능 수학35% / 교과우수 과학 반영
  - 융합보안학과 - 수능 수학35% / 교과우수 과학 반영
  - 미래모빌리티학과 - 수능 수학35% / 교과우수 과학 반영

문과(인문계/사회계) 계열에 유리한 학과:
  - 크리에이티브인문학부 - 수능 국어35% / 교과우수 사회 반영
  - 미래융합사회과학대학 - 수능 국어35% / 교과우수 사회 반영
  - 문학문화콘텐츠학과 - 수능 국어35% / 교과우수 사회 반영
  - 글로벌패션산업학부 - 수능 국어35% / 교과우수 사회 반영

자유 선택(문이과 구분 없음):
  - 상상력인재학부 - 자율전공, 문이과 구분 없이 지원. 수능 우수영역 순 40-30-20-10% 자유반영

예체능/디자인:
  - ICT디자인학부 - 실기 80% 위주 (또는 정시 수능)
  - 예술학부(동양화·서양화·한국무용·현대무용·발레) - 실기 80%
  - 뷰티디자인매니지먼트학과 - 수능 국어35%

※ 교차지원 시 유의사항:
  - 이과생이 문과 학과 지원 시: 교과우수전형에서 사회 교과 성적이 부족할 수 있음
  - 문과생이 이과 학과 지원 시: 교과우수전형에서 과학 교과 성적이 부족할 수 있음
  - 한성인재·지역균형전형은 계열 제한 없이 지원 가능`);
  }
...
- 예술학부 무용(한국무용·현대무용·발레) ← 가군 실기고사, 1월 7일(수)`);
  ...
  if (targetDepts.length > 0) {
    ...
    // RAW_STATS에 실제 존재하는 학과명만 필터
    const validDepts = targetDepts.filter(dept =>
      Object.values(RAW_STATS).some(v => v.모집단위.includes(dept))
    );
    const searchDepts = validDepts.length > 0 ? validDepts : [];

    for (const dept of searchDepts.slice(0, 2)) {
...
교과우수(야간) 기준 8이내: ${best2sum <= 8 ? '✅ 충족' : '❌ 미충족'}`);
  ...
  // 학과 없이 경쟁률/70%컷/전체 요약 요청 시 전체 요약 제공
  if (targetDepts.length === 0 &&
...
⑧ 질문 처리 방침:
   - 학과가 지정된 경우: 해당 학과 데이터 바로 제공
   - 학과 미지정 + 전형 지정 ("교과우수 경쟁률 전체 알려줘"): [검색된 관련 데이터]의 전체 요약 표 제공
   - 학과·전형 모두 미지정 ("경쟁률 어떻게 돼요?"): 주요 전형 전체 요약 제공 후 더 구체적인 학과/전형 질문 유도
   - 모호한 질문에도 가능한 한 데이터를 먼저 보여주고, 마지막에 "특정 학과나 전형을 알려주시면 더 자세히 안내해 드릴 수 있어요."라고 안내
...
⑭ 교차지원 안내 (중요):
   한성대학교는 수시·정시 모두 문과/이과 교차지원이 가능합니다.
   이과 학생도 문과 계열 학과(크리에이티브인문학부 등)에 지원 가능합니다.
   "이과인데 어떤 학과 지원할 수 있나요?" 질문에 이과 유리 학과만 안내하지 말고, 교차지원 가능함을 반드시 안내하세요.

⑮ 학과명 정확성 (매우 중요):
   IT공과대학은 컴퓨터공학부·기계전자공학부·산업시스템공학부를 통합 모집합니다.
   "컴퓨터공학부", "전자공학부", "정보통신공학부", "산업경영공학부" 같은 세부 학과명은 존재하지 않습니다.
   데이터에 없는 학과명을 절대 임의로 만들거나 사용하지 마세요.
   데이터에 있는 학과명만 사용: IT공과대학, 크리에이티브인문학부, 미래융합사회과학대학, 글로벌패션산업학부, 상상력인재학부, AI응용학과, 융합보안학과, 미래모빌리티학과, 문학문화콘텐츠학과, ICT디자인학부, 뷰티디자인매니지먼트학과
